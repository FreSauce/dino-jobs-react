...
const Register = () => {
	...
	const form = useForm({
		...
		validate: {
			...
			confirmPassword: (value, { password }) => value === password ? null : 'Passwords doesnt match'
		}
	});
						...
						<MediaQuery query="(max-width: 600px)" styles={{ width: '100%' }}>
							<Box sx={{ width: '70%', backgroundColor: '#181818', opacity: 0.9, borderRadius: '15px', color: 'white' }} p={30} pt={15}>
								...
								<form onSubmit={form.onSubmit(handleSubmit)}>
									...
									/>
									<Checkbox mt={10} {...form.getInputProps('toc', { type: 'checkbox' })} label='Keep me signed in' />
									<Button fullWidth type='submit' mt={20} loading={loading}>
										<Text my={20} sx={{ fontWeight: '600', fontSize: '1rem' }}>Sign Up</Text>
									</Button>
								</form>
								<Text mt={10} sx={{ fontWeight: '600', fontSize: '0.9rem' }}>Already have an account?{" "}
									<Link mt={10} style={{ textDecoration: 'none' }} to={'/login'}>
										<Text variant='link' sx={{ display: 'inline' }}>Login</Text>
									</Link>
								</Text>
							</Box>
						</MediaQuery>
